  ...
  <transition name="image-viewer-fade">
    <div class="image-viewer" v-show="visible">
      ...
      <div class="image-viewer-nav">
        <div class="image-viewer-nav-prev" @click="pageChange('prev')">
        ...
        <div class="image-viewer-nav-next" @click="pageChange('next')">
...
export default {
    ...
    methods: {
        ...
        imgSize(recursionWidth, recursionHeight) {
            ...
            if (
                imgSizeAuto.width - 10 > this.config.imgMaxWidth ||
                imgSizeAuto.height - 10 > this.config.imgMaxHeight
            ) {
        ...
        imgSizeAuto(width, height) {
            ...
            if (
                width > this.config.imgMaxWidth ||
                height > this.config.imgMaxHeight
            ) {
                zoomSize =
                    width - this.config.imgMaxWidth >
                    height - this.config.imgMaxHeight
        ...
        imgChange(action) {
            ...
            if (action === "prev") {
                ...
                this.index =
                    action <= 0 ? 0 : action >= length ? length : action;
            }
        },
        pageChange(action) {
            const length = this.images.length - 1;
            if (action === "prev" && this.page * 8 >= 8) this.page--;
                ...
                action === "next" &&
                (this.page * 8 < length && length - this.page * 8 > 8)
            )
                this.page++;
            document.querySelector(
                ".image-viewer-nav-thumb > div:first-child"
            ).style.marginLeft =
                -(
                    document.querySelector(".image-viewer-nav-thumb").width *
                    this.page
                ) + "px";
        },
        close() {
